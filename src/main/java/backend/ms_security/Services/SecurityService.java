...
public class SecurityService {
    ...
    public ApiResponse<Session> register(User newUser, String defaultRoleId) {
        ...
        if (theActualUser == null) {
            // Creamos el nuevo usuario en nuestro sistema.
            theActualUser = this.theUserService.create(newUser).getData();

            // Asiganamos el rol por defecto a este nuevo usuario
            ApiResponse<UserRole> roleAssigned = this.theUserRoleService.addUserRole(theActualUser.getId(), defaultRoleId);
            if (!roleAssigned.isSuccess()) {
                return ApiResponse.error("Usuario registrado pero no se pudo asignar el rol por defecto: " + roleAssigned.getMessage());
            }
            ...
            // Construimos la Session del usuario nuevo
            ...
            // Enviamos el mensaje de bienvenida a el aplicativo
        ...
        return ApiResponse.error("El correo ya está registrado");
    ...
    public ApiResponse<Boolean> permissionsValidation(final HttpServletRequest request, Permission permissionData) {
        ...
        if (result == ValidationResult.PERMISSION_DENIED){
            return ApiResponse.success(false, "Permiso denegado");
        } else if (result == ValidationResult.INVALID_TOKEN) {
            return ApiResponse.success(false,"Token inválido o usuario no encontrado");
        }else {
            return ApiResponse.success(true, "Permiso concedido");
        }
    }
